  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className={`relative w-full max-w-sm rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl transform transition-all ${isDarkMode ? 'bg-slate-800 text-white' : 'bg-white text-slate-800'}`}>
        ...
        <button onClick={onClose} className="absolute top-4 right-4 p-2 opacity-50 hover:opacity-100" aria-label={t('detail.aria.closeDetail')}>
          <X size={24} />
        </button>
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1 opacity-70 font-mono text-sm" aria-label={t('detail.aria.schedule', { start: activity.start, end: activity.end })}>
        ...
        <div className="space-y-4">
          <div className={`p-4 rounded-xl ${isDarkMode ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
            <h3 className="text-xs uppercase font-bold tracking-wider opacity-50 mb-2 flex items-center gap-2">
              <Info size={14} /> {t('activity.description')}
            </h3>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className={`p-3 rounded-xl text-center ${isDarkMode ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
              <span className="block text-xs uppercase opacity-50 mb-1">{t('activity.duration')}</span>
              <span className="font-bold">{duration} h</span>
            </div>
            <div className={`p-3 rounded-xl text-center ${isDarkMode ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
              <span className="block text-xs uppercase opacity-50 mb-1">{t('activity.type')}</span>
